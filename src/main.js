...
async function createWindow() {
  ...
  if (globals.discordEnabled) {
    ...
    async function updateDiscordPresence() {
      globals.mainWindow.webContents
        .executeJavaScript("window.gameInfo", true)
        .then((gameInfo) => {
          ...
          if (gameData.gameMode === "Title") {
            ...
            rpc.setActivity({
              ...
              largeImageKey: gameData.biome
                ? gameData.biome.toLowerCase().replace(/\s/g, "_") + "_discord"
                : "logo2",
