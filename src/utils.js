...
function createWindow(opts) {
  ...
  if (opts.enableDeveloperConsole === true) win.webContents.toggleDevTools();
  return win;
}
...
function createPopup(opts, content) {
  ...
  if (content) {
    ...
    window.loadURL(`data:text/html,${encodeURIComponent(htmlContent)}`);
  }
  return window;
}
...
function fetchLatestAppVersionInfo(opts) {
  ...
  return new Promise((resolve, reject) => {
    https
      .get(globals.latestAppReleaseUrl, options, (response) => {
        let data = "";
...
function fetchLatestGameVersionInfo(opts) {
  ...
  return new Promise((resolve, reject) => {
    https
      .get(globals.latestGameReleaseUrl, options, (response) => {
        let data = "";
...
function loadSettings() {
  ...
  if (fs.existsSync(settingsFilePath)) {
    try {
      ...
      globals.isOfflineMode = globals.gameFilesDownloaded
        ? settings.isOfflineMode
        : false;
      globals.mainWindow.webContents.send("offline-mode-status", [
        globals.isOfflineMode,
        globals.gameDir,
      ]);
...
function downloadFile(url, outputPath, onBytesReceived, opts) {
  ...
  return new Promise((resolve, reject) => {
    https
      .get(url, downloadOptions, (response) => {
        if (response.statusCode === 302) {
          const redirectUrl = response.headers.location;
          https.get(redirectUrl, downloadOptions, (response) => {
            response.on("data", (chunk) => {
