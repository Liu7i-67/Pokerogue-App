...
const getTabData = () => {
  ...
  async function createWikiWindow() {
    ...
    const wikiWindowBlocker = await ElectronBlocker.fromPrebuiltAdsAndTracking(
      fetch
    );
    wikiWindowBlocker.enableBlockingInSession(
      globals.wikiWindow.webContents.session
    );
  ...
  async function createPokedexWindow() {
    ...
    const pokedexWindowBlocker =
      await ElectronBlocker.fromPrebuiltAdsAndTracking(fetch);
    pokedexWindowBlocker.enableBlockingInSession(
      globals.pokedexWindow.webContents.session
    );

    globals.pokedexWindow.loadURL(
      "https://ydarissep.github.io/PokeRogue-Pokedex"
    );
